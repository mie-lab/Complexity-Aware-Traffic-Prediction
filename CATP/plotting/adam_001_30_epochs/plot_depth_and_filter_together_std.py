...
for List_of_depths, List_of_filters in [
    ...
    for _, DEP in enumerate(List_of_depths):
        ...
        for _, FIL in enumerate(List_of_filters):
            ...
            for idx, file in enumerate(files.keys()):
                ...
                n = 1
                data["MC"] = data["CSR_MP_sum"] + data["CSR_MP_std"]  
                data["IC"] = data["CSR_PM_sum"].max() + data["CSR_PM_std"].max()
    ...
    def confidence_ellipse(x, y, ax, n_sum=1, facecolor='none', **kwargs):
        """
        ...
        scale_x = np.sqrt(cov[0, 0]) * n_sum
        mean_x = np.mean(x)

        # calculating the standard deviation of y ...
        scale_y = np.sqrt(cov[1, 1]) * n_sum
        mean_y = np.mean(y)
