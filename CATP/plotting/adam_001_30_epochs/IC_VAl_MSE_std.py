...
for file in files:
    ...
    df["IC"] = df["CSR_PM_sum"][df["CSR_PM_sum"] > 0].mean() + df["CSR_PM_std"][df["CSR_PM_std"] > 0].mean()
    df["MC"] = df["CSR_MP_sum"] + df["CSR_MP_std"]
    df["MC_min"] = df["MC"].min()
    df["MC_max"] = df["MC"].max()
...
plt.tight_layout()
# plt.yscale("log")
plt.savefig("combined_plot_val_loss_vs_task_complexity__scatter_std.png")
plt.show()
