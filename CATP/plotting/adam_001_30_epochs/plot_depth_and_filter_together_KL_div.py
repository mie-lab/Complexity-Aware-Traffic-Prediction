    ...
    for _, DEP in enumerate(List_of_depths):
        ...
        for _, FIL in enumerate(List_of_filters):
            ...
            for idx, file in enumerate(files.keys()):
                ...
                mean_IC_mean = data["CSR_PM_sum"][data["CSR_PM_sum"]>0].mean()
                mean_IC_std = data["CSR_PM_std"][data["CSR_PM_sum"] > 0].mean()
                data["IC"] = mean_IC_mean
                data["IC_std"] = mean_IC_std

                mu1 = data["IC"]
                mu2 = data["MC"]
                sigma_1 = data["IC_std"]
                sigma_2 = data["CSR_MP_std"]

                data["KL-div"] = np.log(sigma_2/sigma_1) + ((sigma_1**2 + (mu2 - mu1)**2) / (2*sigma_2**2)) - 0.5
    ...
    for _, DEP in enumerate(List_of_depths):
        ...
        for _, FIL in enumerate(List_of_filters):
            ...
            for idx, file in enumerate(files.keys()):
                ...
                argmin = np.argmin(data["val_loss"])
                plt.scatter(data["KL-div"][20:], data["val_loss"][20:],
                # plt.scatter(data["KL-div"][argmin] , data["val_loss"][argmin],
