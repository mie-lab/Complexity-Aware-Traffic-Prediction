...
head -n 1 london-1-6-55-validation_csv_hard_1.csv > combined.csv
...
for i in $(seq 1 $max_num); do
    if [ -e "london-1-6-55-validation_csv_hard_$i.csv" ]; then
        tail -n +2 "london-1-6-55-validation_csv_hard_$i.csv" >> combined_temp.csv
    fi

    if [ -e "london-1-1-55-validation_csv_easy_$i.csv" ]; then
        tail -n +2 "london-1-1-55-validation_csv_easy_$i.csv" >> combined_temp.csv
    fi
done
