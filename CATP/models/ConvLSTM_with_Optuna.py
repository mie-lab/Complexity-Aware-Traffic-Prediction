...
class ConvLSTM:
    ...
    @staticmethod
    def optuna_objective(trial, cityname, i_o_length, pred_horiz, scale):
        ...
        model = ConvLSTM(
            ...
            scale=scale,
            shape=(2, i_o_length, scale, scale, 1),
            validation_csv_file=obj.key_dimensions() + "validation.csv",
            log_dir=obj.key_dimensions() + "log_dir",
        )
        print(model.model.summary())
        ...
        predictions_folder = os.path.join(config.HOME_FOLDER, "predictions_folder", model.prefix)
        train_data_folder = model.train_data_folder
        validation_data_folder = model.validation_data_folder
        prefix = model.prefix
        cityname = model.cityname
        io_length = model.io_length
        pred_horiz = model.pred_horiz
        scale = model.scale
        ...
        beta_1 = trial.suggest_float('beta_1', 0.8, 0.98)
        ...
        # manual reset the model since sometimes it does not reset a new model (not sure why)
        tensorflow.keras.backend.clear_session()
        model.compile(optimizer=optimizer, loss=loss_fn, metrics=non_zero_mse)

        num_train = len(glob.glob(os.path.join(config.HOME_FOLDER, train_data_folder) + "/" + prefix + "*_x.npy"))
        num_validation = len(
        ...
        prediction_gen = CustomDataGenerator(
            cityname,
            io_length,
            pred_horiz,
            scale,
            data_dir=predictions_folder,
            num_samples=int(num_train * r),
            batch_size=batch_size,
            shuffle=True,
        )

        # csv_logger = CSVLogger(validation_csv_file)

        model.train_gen = train_gen
        model.validation_gen = validation_gen
        model.predict_gen = prediction_gen
        model.predictions_folder = predictions_folder
        model.prefix = prefix
        model.fit(
            train_gen,
        ...
        list_of_errors = []
        for x, y in model.train_gen:
            list_of_errors.append(np.mean( (model.predict(x) - y) ** 2) )

            # lets do it for 10 epochs
            if len(list_of_errors) > 10:
                break
    ...
    for city in config.city_list_def:
        for pred_horiz in config.pred_horiz:
            for scale in config.scales_def:
                for io_len in config.i_o_lengths_def:
                    study = optuna.create_study(direction='minimize')
    
                    custom_objective_partial = functools.partial(ConvLSTM.optuna_objective,
                                                                 cityname=city,
                                                                 scale=scale,
                                                                 pred_horiz=pred_horiz,
                                                                 i_o_length=io_len)
                    ...
                    obj = ProcessRaw(cityname=city, i_o_length=io_len,
                    ...
                    model = ConvLSTM(
                        ...
                        pred_horiz=pred_horiz,
                        scale=scale,
                        shape=(2, io_len,scale, scale, 1),
                        validation_csv_file=obj.key_dimensions() + "validation.csv",
                        log_dir=obj.key_dimensions() + "log_dir",
                    )
                    print(model.model.summary())
                    print(model)
                    sprint (obj.key_dimensions, study.best_params)
